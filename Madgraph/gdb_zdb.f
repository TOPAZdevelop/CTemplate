...
C  
C FOR PROCESS : g d~ -> z d~  
C  
C Crossing   1 is g d~ -> z d~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C             WRITE(*,*) IHEL,T
      ...
      REAL*8 FUNCTION GDB_ZDB(P,NHEL,IC)
C  
C FUNCTION GENERATED BY MADGRAPH
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL FVOXXX(W(1,2   ),W(1,3   ),GZD ,ZERO    ,ZERO    ,W(1,5   ))    
      CALL IOVXXX(W(1,4   ),W(1,5   ),W(1,1   ),GG ,AMP(1   ))             
      CALL FVOXXX(W(1,2   ),W(1,1   ),GG ,ZERO    ,ZERO    ,W(1,6   ))     
      CALL IOVXXX(W(1,4   ),W(1,6   ),W(1,3   ),GZD ,AMP(2   ))            
      JAMP(   1) = -AMP(   1)-AMP(   2)
      GDB_ZDB = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          GDB_ZDB =GDB_ZDB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
